...
def piggy_points(score):
    ...
    while(x > 0):
        rightMost = x % 10
        x = (x-rightMost) / 10
        if rightMost < lowestDigit:
            lowestDigit = rightMost
    rst += int(lowestDigit)
    return rst
    # END PROBLEM 2
...
def more_boar(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    def str_of_score(score: int) -> str:
        rst = str(score)
        if len(rst) < 2:
            rst = '0' + rst
        return rst

    player_str = str_of_score(player_score)
    opponent_str = str_of_score(opponent_score)

    for i in range(2):
        if player_str[i] >= opponent_str[i]:
            return False
    return True
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def getRstOfWhosStrategy(who = who):
        if who == 0:
            return strategy0(rst[who], rst[nextP])
        else:
            return strategy1(rst[who], rst[nextP])

    def check(goal = goal):
        for i in rst:
            if i >= goal:       # the condition that game should over
                return True
        return False
    ...
    while True:
        nextP = next_player(who)
        isMoreBoar = True      #to realize 'do-while' in C
        while isMoreBoar:
